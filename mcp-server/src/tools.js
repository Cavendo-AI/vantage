/**
 * Vantage MCP tool registrations — extracted so both stdio and HTTP
...
export function registerTools(server, client) {
    ...
    async ({ content, signal_type, platform, title, source_url, source_name, source_bio, source_org, source_handle, published_at, topics, importance, sentiment }) => {
      try {
        ...
        const signal = await client.captureSignal({
          sourceId, signalType: signal_type, platform, title, content,
          sourceUrl: source_url, publishedAt: published_at, topics, importance, sentiment
        });
  ...
  server.tool(
    ...
    async ({ query, platform, topic, importance, since, until, limit }) => {
      try {
        const data = await client.listSignals({ q: query, platform, topic, importance, since, until, limit });
        const signals = data.signals || data;
        if (!signals.length) return { content: [{ type: 'text', text: 'No signals found.' }] };
        return { content: [{ type: 'text', text: `Found ${signals.length} signals:\n\n${signals.map(formatSignalBrief).join('\n\n---\n\n')}` }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
      }
    }
  ...
  server.tool(
    ...
    async ({ limit, platform, topic, since }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `${data.length} recent signals:\n\n${data.map(formatSignalBrief).join('\n\n---\n\n')}` }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
      }
    }
  ...
  server.tool(
    ...
    async ({ name, bio, organization, credibility, platform_handles, notes }) => {
      try {
        ...
        let source;
        if (existing) {
          source = await client.updateSource(existing.id, { name, bio, organization, credibility, platformHandles: platform_handles, notes });
        } else {
          source = await client.createSource({ name, bio, organization, credibility, platformHandles: platform_handles, notes });
        }
        return {
  ...
  server.tool(
    ...
    async ({ limit }) => {
      try {
        ...
        const lines = sources.map(s => `- **${s.name}** (${s.organization || 'N/A'}) — ${s.credibility} — ${s.signalCount || 0} signals`);
  ...
  server.tool(
    ...
    async ({ name, description, color }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `Topic ${existing ? 'updated' : 'created'}: ${topic.name} (${topic.slug})` }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
      }
    }
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const lines = topics.map(t => `- **${t.name}** (${t.signalCount || 0} signals)${t.description ? ` — ${t.description}` : ''}`);
  ...
  server.tool(
    ...
    async ({ title, context_type, content }) => {
      try {
        const ctx = await client.createContext({ title, contextType: context_type, content });
    ...
    async ({ analysis_type, collection_id, topic, signal_ids, business_context_id, question }) => {
      try {
        ...
        if (collection_id) {
          ...
          for (const id of signal_ids) { signals.push(await client.getSignal(id)); }
        } else if (topic) {
          const data = await client.listSignals({ topic, limit: 100 });
          signals = data.signals || data;
        } else {
          signals = await client.getSignalFeed(50);
        }
        if (business_context_id) { contextDoc = await client.getContext(business_context_id); }

        let output = `# Analysis Request: ${analysis_type}\n\n`;
        if (question) output += `**Question:** ${question}\n\n`;
        if (contextDoc) {
          output += `## Business Context: ${contextDoc.title}\nType: ${contextDoc.contextType}\n\n${contextDoc.content}\n\n`;
        }
        ...
        for (const s of signals) {
          ...
          if (s.topics?.length) output += `- Topics: ${s.topics.map(t => t.name || t).join(', ')}\n`;
    ...
    async ({ title, analysis_type, content, signal_ids, business_context_id, collection_id }) => {
      try {
        const analysis = await client.saveAnalysis({
          title, analysisType: analysis_type, content,
          signalIds: signal_ids, businessContextId: business_context_id, collectionId: collection_id
        });
